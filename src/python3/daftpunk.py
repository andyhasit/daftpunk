...
p = lambda lines, repeat=1: mp(lambda x: mp(print, [line.capitalize() for line in lines] + ['']), range(repeat))
mrg = lambda *x: [' '.join(i) for i in zip(*x)]
j = lambda *x: ' '.join(x)
mp(p, (v, a, m, e, v, a))
mp(lambda x: p(mrg(*x)), ((v, a), (m, e), (v, a)))
p(mrg(v, a), repeat=5)
...
mp(lambda x: mp(p, [[wihmi, difmu, mteh, owi], [wihmi, difmu, mteh, owino], mrg(v, a)]), range(2))
